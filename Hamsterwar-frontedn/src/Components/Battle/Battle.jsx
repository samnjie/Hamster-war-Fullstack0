...
const Battle = (props) => {
  ...
  useEffect(() => {
    const getHamsters = async () => {
      ...
      do {
        for (let i = 0; i <= 1; i++) {
          const response = await fetch("https://hamsters-backend-90f4de1edeea.herokuapp.com/hamsters/random");
  ...
  const vote = async (id) => {
    ...
    try {
      await Promise.all([
        fetch(
          `https://hamsters-backend-90f4de1edeea.herokuapp.com/hamsters/${winnerHamsterId}`,
          winnerRequest
        ),
        fetch(
          `https://hamsters-backend-90f4de1edeea.herokuapp.com/hamsters/${loosingHamsterId}`,
          looserRequest
        ),
      ]);
      setVoted(true);
